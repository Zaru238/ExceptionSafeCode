...
TEST (ValgrindTest, MemoryLeak) {
  try {
    size_t* leakedPointer = new size_t(0);
  } catch (...) {
  }
}
...
TEST (MemoryLeakOnDeepCopy, ThrowOnFirstCopy) {
  try {
    using SpecializedAggregator = Aggregator<ThrowOnCopyResource, NonThrowOnCopyResource, NonThrowOnCopy>;
...
TEST (MemoryLeakOnDeepCopy, ThrowOnSecondCopy) {
  try {
    using SpecializedAggregator = Aggregator<NonThrowOnCopyResource, ThrowOnCopyResource, NonThrowOnCopy>;
    SpecializedAggregator first;
    SpecializedAggregator second (first);
  } catch (...) {
  }
...
TEST (MemoryLeakOnDeepCopy, ThrowAfterTwoCopies) {
  try {
    using SpecializedAggregator = Aggregator<NonThrowOnCopyResource, NonThrowOnCopyResource, ThrowOnCopy>;
    SpecializedAggregator first;
    SpecializedAggregator second (first);
  } catch (...) {
  }
